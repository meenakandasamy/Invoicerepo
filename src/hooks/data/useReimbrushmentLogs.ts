...
export const useReimbrushmentLogs = (id: string | number, session: Session) => {
  // console.log(id, 'idTest');
  const { data: userList } = useUserList(session);

  return useQuery<Array<any>, Error>({
    queryKey: [EmployeeQueries.GET_REIMBRUSHMENT_LOGS_BY_ID, id],
    queryFn: async () => {
      if (!id || !userList) return [];

      try {
        const response = await EmployeeServices.fetchReimbrushmentLogsById(id);
        ...
        throw new Error(error?.message || 'Error fetching reimbrushment logs');
      }
    },
    enabled: !!id,
    retry: 1,
    meta: {
      successMessage: 'Reimbrushment logs loaded successfully!',
      toastSuccess: false,
      errorMessage: 'Failed to load reimbrushment logs',
    },
    staleTime: 5 * 60 * 1000,
    gcTime: 30 * 60 * 1000,
  });
