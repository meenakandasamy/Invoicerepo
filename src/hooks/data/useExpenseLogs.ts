...
export const useExpenseLogs = (
  ...
  return useQuery<Array<ExpenseLogsTypes>, Error>({
    queryKey: [ExpenseQueries.GET_LOGS_BY_ID, id],
    queryFn: async () => {
      if (!id || !userList) return [];

      try {
        const response = await ExpenseServices.fetchExpenseLogsById(id);
        ...
        throw new Error(error?.message || 'Error fetching expense logs');
      }
    },
    enabled: !!id && userList !== undefined,
    retry: 1,
    meta: {
      successMessage: 'Expense logs loaded successfully!',
      toastSuccess: false,
      errorMessage: 'Failed to load expense logs',
    },
    staleTime: 5 * 60 * 1000,
    gcTime: 30 * 60 * 1000,
  });
