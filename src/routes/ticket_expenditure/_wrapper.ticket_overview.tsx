...
function RouteComponent() {
  ...
  const putholdMutation = useMutationFn(
    ...
    const putcloseMutation = useMutationFn(
    TicketconfigServices.UpdateTicketClose,
    TicketconfigQueries.GET_TICKET_CLOSE,
    //  TicketconfigQueries.GET_TICKET_DETAILS,
  );
  const defaultValues = {
    date: '',
  ...
  const [formFields, setFormFields] =
    useState<viewticketFiledType>(defaultValues);
  ...
  async function onSubmitdata(data: any) {
    ...
    downloadFile(
      response.data,
      `Ticket_${urlResponse?.ticketId}.pdf`
    );
  } catch (error) {
    ...
    if (selectdata === 'reassign') {
      const datas = {
        ...
        scheduleOn:
          new Date(data.date).toISOString().split('T')[0] + ' ' + data.timeslot,
    ...
    }else if (selectdata === 'close') {
      const datas = {
        remarks: data.remarks,
        lastUpdatedBy: session.userId,
        ticketId: urlResponse?.ticketId,
        base64File:data.document
      };

      putcloseMutation.mutate(datas, {
        onSuccess: () => {
          setToBackend(false);
  ...
  console.log(ticketdetails.statusName);
  
  const access = { hasCreateAccess: true, hasUpdateAccess: true };
  const hide = {
reassign:
  ticketdetails.statusName === "Finished" ||
  ticketdetails.statusName === "Hold"
    ? true
    : false,
    hold: 
  ticketdetails.statusName === "Finished"||  ticketdetails.statusName === "Hold"||  ticketdetails.statusName === "Unfinished"
    ? true
    : false,
    close:  ticketdetails.statusName === "Finished"||ticketdetails.statusName === "Unfinished"?false:true,
    download: ticketdetails.statusName === "Finished"||ticketdetails.statusName === "Unfinished"?false:true,
  };
  return (
    <div className="p-4 h-6">
      ...
      <TicketDetailsCard ticketdetails={ticketdetails} />
      <TicketActivityTable ticketHistory={ticketHistory} />
      {isOpen && (
